...
/**
 * 特定ユーザーの関係性メトリクスを自動的にクリーンアップするスクリプト
 * trust_level と emotional_connection を NULL に設定
 */
async function migrateIndividualUserAuto(email: string) {
  const client = await pool.connect();
  
  try {
    console.log(`\n=== ${email} のデータマイグレーション開始（自動実行） ===\n`);
    ...
    console.log('\n✅ マイグレーション完了！');
    console.log('✅ trust_level と emotional_connection が NULL に設定されました');
    console.log('✅ その他のデータ（メッセージ、メモリ、親密度など）は保持されています');
...
if (!email) {
  console.error('使用方法: npx ts-node scripts/migrate-individual-user-auto.ts metavicer@gmail.com');
  process.exit(1);
}

// 実行
migrateIndividualUserAuto(email).catch(console.error);